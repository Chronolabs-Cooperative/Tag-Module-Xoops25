...
if ($tagConfigsList['htaccess'])
{
	if (is_object($GLOBALS["xoopsModule"]) || "tag" != $GLOBALS["xoopsModule"]->getVar("dirname", "n")) {
		$url = XOOPS_URL . "/" . $tagConfigsList['base'] . "/feed/cat/$start/$sort/$order/$mode/$catid-" . $GLOBALS["xoopsModule"]->getVar("dirname", "n") . $tagConfigsList['rss'];
	} else {
		$url = XOOPS_URL . "/" . $tagConfigsList['base'] . "/feed/cat/$start/$sort/$order/$mode/$catid" . $tagConfigsList['rss'];
	}
